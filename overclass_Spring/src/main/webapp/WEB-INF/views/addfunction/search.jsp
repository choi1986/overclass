...
if (request.getAttribute("selectIdName") != null) {
	...
	int resultCount = (int) request.getAttribute("resultCount");
					%>아이디/이름 검색결과:<%=resultCount%><br><a href="search" id="newsearch">[ 다시검색 ]</a><br><%
	for (int i = 0; i < list.size(); i++) {
					%><hr>
					생일: <input type="text" value="<%=list.get(i).getUser_birth()%>" readonly="readonly" size="10" id="user_birth<%=i + 1%>"><br>
					성별: <input type="text" value="<%=list.get(i).getUser_gender()%>" readonly="readonly" size="10" id="user_gender<%=i + 1%>"><br>
					아이디: <input type="text" value="<%=list.get(i).getUser_id()%>" readonly="readonly" size="10" id="user_id<%=i + 1%>"><br>
					사진: <input type="text" value="<%=list.get(i).getUser_image()%>" readonly="readonly" size="10" id="user_image<%=i + 1%>"><br>
	...
	if (resultCount < 11) {
		...
		if (pageNum<5){
			...
			if (currentPage>0&&currentPage<=lastPage){
				if (currentPage > 1) {
					%><a href="searchIdNamePage?page=1" id="firstPage">[ 첫페이지 ]</a><%
				};
				if (currentPage > 5) {
					%><a href="searchIdNamePage?page=<%=currentPage - 5%>" id="prePage">[ 이전페이지 ]</a><%
				};
				if (currentPage == lastPage) {
					%><a href="searchIdNamePage?page=<%=lastPage%>">[ <%=lastPage%> ]</a><%
					...
					for (int i = 2; i < 7; i++) {
					%><a href="searchIdNamePage?page=<%=i%>">[ <%=i%> ]</a><%
					};
			...
			if (currentPage < lastPage) {
				if (currentPage > lastPage - 5) {
					%><a href="searchIdNamePage?page=<%=lastPage%>" id="nextPage">[ 다음페이지 ]</a><%
				} else {
					%><a href="searchIdNamePage?page=<%=currentPage + 5%>" id="nextPage">[ 다음페이지 ]</a><%
				};
			};
			if (currentPage < lastPage) {
					...
					%>잘못된 요청입니다<a href="main" id="goMain">[ 메인으로 ]</a><%
			};
		};
	};
	...
	for (int i = 0; i < doclist.size(); i++) {
					%><hr>
					글번호: <input type="text" value="<%=doclist.get(i).getDno()%>" readonly="readonly" size="10" id="doc_dno<%=i + 1%>"><br>
	...
	if (resultCount < 11) {
		...
		if (pageNum<5){
			...
			if (currentPage>0&&currentPage<=lastPage){
				if (currentPage > 1) {
					%><a href="searchTagPage?page=1" id="firstPage">[ 첫페이지 ]</a><%
				};
				if (currentPage > 5) {
					%><a href="searchTagPage?page=<%=currentPage - 5%>" id="prePage">[ 이전페이지 ]</a><%
				};
				if (currentPage == lastPage) {
					%><a href="searchTagPage?page=<%=lastPage%>">[ <%=lastPage%> ]</a><%
				} else if (currentPage == 1) {
					for (int i = 0; i < 5; i++) {
					%><a href="searchTagPage?page=<%=i + 1%>">[ <%=i + 1%> ]</a><%
					};
					...
					for (int i = 2; i < 7; i++) {
					%><a href="searchTagPage?page=<%=i%>">[ <%=i%> ]</a><%
					};
				} else if (currentPage==lastPage-1){
				for (int i=lastPage-5;i<lastPage;i++){
					%><a href="searchTagPage?page=<%=i + 1%>">[ <%=i + 1%> ]</a><%
			...
			if (currentPage < lastPage) {
				if (currentPage > lastPage - 5) {
					%><a href="searchTagPage?page=<%=lastPage%>" id="nextPage">[ 다음페이지 ]</a><%
				} else {
					%><a href="searchTagPage?page=<%=currentPage + 5%>" id="nextPage">[ 다음페이지 ]</a><%
				};
			};
			if (currentPage < lastPage) {
